...
public class LangConfig extends ListenerAdapter {
    @Override
    public void onStringSelectInteraction(StringSelectInteractionEvent event) {
        if(!event.getComponentId().equals("config.language.select")) { return; }

        GuildConfig guildConfig = new GuildConfig(Objects.requireNonNull(event.getGuild()).getId());
        guildConfig.setLocale(event.getValues().get(0).toLowerCase());
        guildConfig.update();
